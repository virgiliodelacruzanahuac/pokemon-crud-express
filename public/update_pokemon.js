...
async function updatepokemon() {
  ...
  for ( index = 0; index < inputArray.length; index++)
    {
        if (inputArray[index].type == 'text' && inputArray[index].disabled == false && inputArray[index].name == 'nombre_pokemon')
          {
            nombre_pokemon = inputArray[index].value;
               
            }
           if (inputArray[index].type == 'text' && inputArray[index].disabled == false && inputArray[index].name == 'tipo_pokemon')
          {
            tipo_pokemon = inputArray[index].value;
    ...
    let res = await axios.post('/actualizarpokemones', n);
...
async function ObtienePokemones() {
  ...
  for (var x = 0; x<data.length;x++) {
     var row = table.insertRow();
     var cell1 = row.insertCell();
    cell1.innerHTML = "<input type='radio' name = 'radioid' id='radio_"+ data[x]["clave_pokemon"]  +"' value = "+ data[x]["clave_pokemon"]  +" onclick='activacampos(this)'' />"
    cell1 = row.insertCell();
